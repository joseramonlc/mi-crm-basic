 ...
 */
export function Avatar({ name = '', size = 'md', priority = null, style = {}, ...rest }) {
    ...
    <span style={{ position: 'relative', display: 'inline-flex', flex: 'none', ...style }} {...rest}>
      <span
        style={{
          ...
          display: 'inline-flex', alignItems: 'center', justifyContent: 'center', letterSpacing: '0.01em',
        }}
      >
        {initials || '–'}
      </span>
      {priority && (
        <span
          style={{
            ...
            width: Math.max(10, px * 0.26), height: Math.max(10, px * 0.26),
            borderRadius: '50%', background: `var(--color-priority-${priority}-dot)`,
