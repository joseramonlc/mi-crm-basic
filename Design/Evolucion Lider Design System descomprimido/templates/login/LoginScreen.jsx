// Login — self-contained screen for the Login template.
// Reads design-system primitives from the compiled bundle (loaded via ds-base.js).
// Gated so it renders once the bundle global is ready.
function useNS() {
...
function PasswordField({ Icon, label = 'Contraseña', value, onChange, placeholder = 'Tu contraseña', error }) {
...
function LoginScreen() {
  ...
  const submit = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      if (email.includes('hola')) setDone(true);
      else setError(true);
    }, 700);
  };
          ...
          <div style={{ padding:'24px 0' }}>
            ...
            {error && (
              <div style={{ display:'flex', alignItems:'center', gap:8, background:'var(--color-error-bg)', border:'1px solid #FECACA', borderRadius:'var(--radius-md)', padding:'10px 12px', marginBottom:16 }}>
                <Icon name="alert-circle" size={16} color="var(--color-error-text)" />
                <span style={{ fontSize:13, color:'var(--color-error-text)', fontWeight:500 }}>Email o contraseña incorrectos</span>
              </div>
            )}
            <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
              ...
              <div>
                <PasswordField Icon={Icon} value={pass} onChange={e => { setPass(e.target.value); setError(false); }} error={error ? ' ' : ''} />
                <div style={{ textAlign:'right', marginTop:8, fontSize:13 }}><LinkText>¿Olvidaste tu contraseña?</LinkText></div>
              </div>
              <Button variant="primary" size="lg" fullWidth loading={loading} onClick={submit}>Iniciar sesión</Button>
            </div>
            <div style={{ textAlign:'center', marginTop:24, fontSize:14, color:'var(--color-neutral-500)' }}>
              ¿No tienes cuenta? <LinkText>Regístrate</LinkText>
            </div>
          </div>
        </div>
      )}
