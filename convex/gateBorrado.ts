...
const relleno = (bytes: number) => "x".repeat(Math.max(0, Math.floor(bytes)));

/**
 * Siembra UN prospecto al peor caso medido (paso 1) y sus interacciones. Inserta por
 * `ctx.db.insert` DIRECTO, saltándose la validación de longitudes: así reproduce documentos
 * mayores que los topes actuales (imita históricos previos a JOS-24/JOS-74). Su propia
 * transacción; sus métricas no cuentan (medimos el borrado en `medirBorrado`).
 */
export const sembrarPeorCaso = internalMutation({
  args: {
    usuarioId: v.string(),
    numInteracciones: v.number(),
    bytesProspecto: v.number(),
    bytesInteraccion: v.number(),
  },
  ...
  handler: async (ctx, args) => {
    ...
    const prospectoId = await ctx.db.insert("prospectos", {
      ...
      fechaAlta: Date.now(),
      notas: relleno(args.bytesProspecto),
    });
    for (let i = 0; i < args.numInteracciones; i++) {
      await ctx.db.insert("interacciones", {
        ...
        tipo: "message",
        resultado: "thinking",
        queOcurrio: relleno(args.bytesInteraccion),
      });
    }
    return prospectoId;
  },
