...
import RegistrarInteraccionError from "./error";

afterEach(cleanup);

describe("error.tsx de registrar interacción (contrato del fallback)", () => {
  it("error genérico: 'Reintentar' invoca unstable_retry exactamente una vez", () => {
    const retry = vi.fn();
    const silencio = vi.spyOn(console, "error").mockImplementation(() => {});

    render(<RegistrarInteraccionError error={new Error("boom")} unstable_retry={retry} />);
  ...
  it("NOT_FOUND (id inexistente o ajeno, contrato opaco de M2): sin Reintentar, con vuelta a Inicio", () => {
    ...
    render(<RegistrarInteraccionError error={error} unstable_retry={retry} />);
